...
│                                  # gen_figures.py (the figures, drawn from code)
├── sim/
...
│   ├── assets/figures/            # SVG figures drawn by scripts/gen_figures.py (committed)
│   ├── components/                # Eq, Cite, Figure, Quiz, Mission, ResourceList, tool islands
...
- Every figure is SVG generated by code (matplotlib/Plotly export) or a
  ...
  figures, no textbook figure reproductions. `scripts/gen_figures.py` draws
  the schematics (schemdraw) and idealized waveforms (matplotlib) in the
  site's symbols; `<Figure name="…" />` inlines one in the page's text
  colour with its caption, text alternative and citation
  (`src/lib/figures.ts`), and CI redraws them and fails on a difference.
