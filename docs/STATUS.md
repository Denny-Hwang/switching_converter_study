...
| Symbol meanings: every symbol of `symbol_table` says in a few words what it is (`meaning`, `meaning_ko`, required by the loader) | ✅ 150 of 150; shown next to every tool input (readability pass), next to the equations to follow |
...
| Screenshots on tool pages (`scripts/screenshots.mjs`, checked by `modulelint.py`) | ✅ explorer, simulator, converter designer, loss budget, clamp check, source matcher, sense chain |
| Keyboard focus order of tool pages (`scripts/keyboard_check.mjs`, CI) | ✅ explorer, simulator, converter designer, loss budget, clamp check, source matcher, sense chain |
...
_Last updated: readability pass of the tools (after Phase 3d). `python scripts/modulelint.py` checks every ✅ below against the pages themselves._
