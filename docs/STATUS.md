...
| Operating modes: the steady period split into modes, each with its circuit (branches carrying current coloured, arrows in the current's direction, the switch and its body diode as they conduct, each element's name and state), a description and a table of every element's state, current and voltage; the charts shade the selected mode | ✅ EN and KO: Kirchhoff's current law at every node of the drawn circuit at every sample (every topology × load × node capacitance × source, `sim-schematic.test.ts`), states against hand-checked cases (`sim-sequence.test.ts`), a text for every mode, element and state in both languages, and energy sentences written from the table's states (`sequence.test.ts`), no overlap in the drawing for any circuit in either language (`seqlayout.test.ts` with estimated text widths, `scripts/seq_check.mjs` with the browser's own in CI), rounding neither a current nor a mode (a current below a thousandth of its scale counts as none: the switching cell's largest in the mode, or a load's or source's element's own peak; each mode's circuit sentence agrees with its states in the test circuits and in 80 seeded random ones, a mode shorter than a billionth of the period joins its neighbour, a circuit at rest shows no modes), the instant each current ends within 3 ns of ngspice (2.1 ns at worst, `sim-spice.test.ts`) |
