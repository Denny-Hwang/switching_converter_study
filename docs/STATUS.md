...
| Symbol meanings: every symbol of `symbol_table` says in a few words what it is (`meaning`, `meaning_ko`, required by the loader) | ✅ 150 of 150; shown next to every tool input, under every `<Eq>` and in every worked example |
...
_Last updated: readability pass of the pages (after Phase 3d). `python scripts/modulelint.py` checks every ✅ below against the pages themselves._
