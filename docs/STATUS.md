...
| Cross-check against ngspice (`scripts/spice_crosscheck.py`, `sim-spice.test.ts`) | ✅ 23 circuits rebuilt as netlists from their schematics: every topology in CCM and DCM, the forward converter's intervals, node-capacitance ringing, a Thevenin source, batteries with and without a resistor, a fixed output, a runaway start-up, capacitors charging. Averages, extremes, end values and 40 waveform instants per quantity agree within 0.5 % of its largest magnitude; a CI job reruns ngspice against the stored results |
| Invariants with every load (`sim-invariants.test.ts`) | ✅ five converters × R ∥ C, a battery, a battery and R × ideal and lossy: charge and volt-second balance, i_C = C dv/dt, source power = load power + losses; energy conserved over start-ups from rest |
...
| No steady state: the simulator says which state keeps changing and by how much per cycle (the average inductor voltage, the balancing duty ratio; a capacitor's gain per cycle) and shows the start-up from rest | ✅ EN and KO; runaway, charging and unsettled cases tested |
