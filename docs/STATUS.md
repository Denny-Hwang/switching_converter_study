...
| CircuitJS1 additions (Phase 5b) | ✅ 1 equation: `sim.step_jump` (the voltage a backward Euler step puts across an inductor whose current it has to stop, the spike a fixed-step simulator makes where a diode's current has nowhere to go) |
...
| Tool fields (`src/lib/siparse.ts`, `NumInput`): values as a datasheet writes them, in all eight tools: an SI prefix, the field's unit, or both (100u, 100 µH, 200 kHz, 0.3 T), a percentage, a Korean keyboard's unit symbols (NFKC); text that is not a number marked invalid; a new load or source fills the fields it adds from an example | ✅ EN and KO: the parser's forms, refusals and exact scaling, unit by unit (`siparse.test.ts`); prefixed values give the same parameters as base units; the fill for every load and source of every preset (`Simulator.test.ts`); `keyboard_check.mjs` edits the text fields |
...
| Every mode at once (`ModeSheet`): the period as a paper lays it out, the key waveforms of each topology on one time axis (gate, inductor or magnetizing current, switch and diode currents, switch voltage, inductor voltage; the forward converter's i_M, i_D1, i_D2) with the boundaries t_0 … t_N dashed and each row's extremes, then every mode's circuit and description, lettered (a), (b), …; in the simulator for the form's values, and on the five topology pages for their examples (the SPICE library's seven cases), simulated at build time | ✅ EN and KO: one figure per mode in order, a trace per row from the simulated series, the boundaries at the modes' instants, the gate on exactly in the modes whose gate is on and for D T_s (`ModeSheet.test.ts`, the seven cases and two battery loads); an example without a steady state fails the build |
...
| LTspice `.asc`, ngspice `.cir` and Falstad links on the simulator page | ✅ LTspice and ngspice (Phase 5a): the simulator page links each preset's schematic and netlist (tested, EN and KO); CircuitJS1 (Phase 5b): each preset's share link, from the generated `src/generated/falstad.json` (tested) |
...
| CircuitJS1 circuits (`sim/falstad/`, `scripts/falstad_library.py`): the SPICE library's seven cases for Paul Falstad's browser simulator, as circuit files and share links, with a body diode across each switch, backward Euler magnetics and a coupling of 0.99999; each opens at about one switching period a second with slow current dots, starting from the ideal steady state's values | ✅ Phase 5b: generated from the SPICE library's list of parts; each circuit's wiring checked against its netlist, node by node (tested, with miswirings it must catch); the links' compression checked against lz-string's own output (tested); CI (job `falstad`) opens a copy of every link on falstad.com, the same circuit with only its speed raised (tested), in a headless browser and compares the averages and extremes of its last two periods with the ideal equations, within 1 % of each quantity's scale; a local build of CircuitJS1 at the same version gave the same numbers |
| CircuitJS1 from the simulator's form (`src/lib/falstadgen.ts`, the simulator's "Open these values in CircuitJS1"): the same circuit for any values with a resistive load and no source, R_on from its field, starting at the simulated steady state | ✅ the TypeScript generator writes the library's seven circuit files and links character for character (`falstadgen.test.ts`), so the netlist wiring check and the CI's CircuitJS1 runs cover it; the start within the output ripple and 1 % of the ideal values; five circuits of other values run in a local CircuitJS1 build within 0.5 % of the simulator's average output |
...
_Last updated: Phase 5b (the CircuitJS1 circuits). `python scripts/modulelint.py` checks every ✅ below against the pages themselves._
...
| falstad | 5 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | homes `sim.step_jump`; the library's CircuitJS1 circuits and links, CircuitJS1's fixed step, and the spike where a diode's current has nowhere to go |
...
| 02-theory/derivations | ✅ | ✅ | auto-rendered from `python/pe_core/derive/*.py`: 19 modules, 162 derived equations (Phase 5b) |
