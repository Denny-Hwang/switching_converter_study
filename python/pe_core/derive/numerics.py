...
def derive() -> Derivation:
    ...
    d.step(
        "With its source at rest (a voltage source is a short), the filter's inductance, its capacitance and the "
        "load resistance are in parallel. The natural response of this parallel RLC circuit goes as $e^{st}$, "
        "with $s$ a root of its characteristic equation:",
        "전원이 멈춘 상태에서(전압원은 단락) 필터의 인덕턴스, 커패시턴스, 부하 저항은 병렬이다. 이 병렬 RLC 회로의 "
        "자연 응답은 $e^{st}$를 따르며, $s$는 특성 방정식의 근이다.",
        sp.Eq(char, 0),
    )
    roots = sp.solve(char, s)
    alpha = sp.simplify(-(roots[0] + roots[1]) / 2)
    d.step(
        "When the filter rings, the roots are a complex-conjugate pair, and their common real part, half their "
        "sum, is the decay rate of the ring's envelope (the neper frequency):",
        "필터가 진동할 때 두 근은 켤레 복소수이며, 공통 실수부, 곧 두 근의 합의 절반이 링잉 포락선의 감쇠율(네퍼 "
        "주파수)이다.",
    ...
    d.result(
        ...
        "포락선은 $e^{-\\alpha t}$로 감쇠하며, 시정수는 인덕턴스와 관계없이 $1/\\alpha$이다.",
        S("tau_env"),
    )
