...
- a body diode across each switch, as the site's simulator has: CircuitJS1 steps with a fixed
  time step, so a diode's current overshoots zero by up to a step's change when it turns off,
  and the node that current leaves behind would jump far past the rails;
...
- a coupling of 0.99999 instead of 1: CircuitJS1 inverts the windings' inductance matrix,
  which is singular at 1. The leakage left, 2e-5 of the magnetizing inductance, adds a spike
  of about a volt at turn-off.
