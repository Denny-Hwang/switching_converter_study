...
# fmt: off
CASES: list[dict] = [
    # id, topology, the synthetic example and the simulator preset that opens it (its name in the
    # preset menu, src/lib/simpresets.ts), periods simulated
    {"id": "buck-ccm", "topology": "buck", "example": "buck-basic", "preset": ("buck", "Buck, CCM", "벅, CCM"), "cycles": 400},
    {"id": "buck-dcm", "topology": "buck", "example": "buck-light-load", "preset": ("buck-light", "Buck, light load (DCM)", "벅, 경부하(DCM)"), "cycles": 1500},
    {"id": "boost-ccm", "topology": "boost", "example": "boost-ideal", "preset": ("boost", "Boost", "부스트"), "cycles": 800},
    {"id": "buckboost-ccm", "topology": "buckboost", "example": "buckboost-basic", "preset": ("buckboost", "Buck-boost", "벅-부스트"), "cycles": 1200},
    {"id": "flyback-ccm", "topology": "flyback", "example": "flyback-ccm", "preset": ("flyback-ccm", "Flyback, CCM", "플라이백, CCM"), "cycles": 1500},
    {"id": "flyback-dcm", "topology": "flyback", "example": "flyback-dcm", "preset": ("flyback-dcm", "Flyback, DCM", "플라이백, DCM"), "cycles": 2500},
    {"id": "forward-ccm", "topology": "forward", "example": "forward-basic", "preset": ("forward", "Forward", "포워드"), "cycles": 1200},
]
# fmt: on
...
def parts(case: dict) -> list[Part]:
    ...
    if t == "buck":
        ...
        out += [
            ...
            # current to pass from the primary to the reset winding at turn-off (0.01 pF is too
            # little for ngspice). After the reset, the magnetizing current rings once into it and
            # the forward diode then holds what is left, sqrt(Cd/Lm) Vg (1.5 mA here), through the
            # idle interval: less than 1 % of the magnetizing current's peak.
...
def netlist(case: dict) -> str:
    ...
    lines = [
        ...
        f"* the numbers of the in-browser simulator's preset \"{case['preset'][1]}\".",
...
def schematic(case: dict) -> str:
    ...
    notes = [
        f"{title(case)}: {ex['label']} (examples/synthetic/{case['example']}.yaml),",
        f"the numbers of the in-browser simulator's preset \"{case['preset'][1]}\". Near-ideal parts, not from any design.",
...
def readme(topology: str, tool: str, results: dict) -> str:
    ...
    for ko in (False, True):
        ...
        for c in cases:
            ...
            out.append(f"| [{c['id']}.{ext}]({c['id']}.{ext}) | {example_text(c, ko)} | {mode} | {c['preset'][2 if ko else 1]} |")
        out.append("")
        head = "#### " if ko else "## "
        if tool == "ltspice":
            if ko:
        ...
        if ko:
            ...
            if topology == "forward":
                out.append(
                    ...
                    "SPICE에 드레인 커패시턴스가 필요합니다. 리셋이 끝나면 자화 전류가 한 번 Cd와 공진하고, 순방향 다이오드가 남은 전류 "
                    "sqrt(Cd/L_M)·V_g(여기서 약 1.5 mA)를 휴지 구간 동안 붙잡아 둡니다. 자화 전류 최댓값의 1 %보다 작습니다."
                )
        else:
            out += [
                f"{head}Notes",
            ...
            if topology == "forward":
                out.append(
                    ...
                    "for the current to pass from the primary to the reset winding at turn-off. After the reset, the magnetizing current rings once "
                    "into it, and the forward diode then holds what is left, sqrt(Cd/L_M) V_g (about 1.5 mA here), through the idle interval: "
                    "less than 1 % of the magnetizing current's peak."
                )
        out.append("")
    return "\n".join(out).rstrip() + "\n"
...
def sim_readme() -> str:
    ...
    for c in CASES:
        t = c["topology"]
        files = (
            f"[{c['id']}.asc](ltspice/{t}/{c['id']}.asc) | [{c['id']}.cir](ngspice/{t}/{c['id']}.cir) | "
            f"[{c['id']}.txt](falstad/{c['id']}.txt) | [svg](waveforms/{c['id']}.svg)"
        )
    ...
    return "\n".join(
        [
            ...
            *rows,
            "",
            "Every LTspice, ngspice and waveform file is generated by `scripts/sim_library.py` from one list of parts, so a schematic and its netlist are the same circuit; "
            ...
            "",
            "The CircuitJS1 circuits, and the links that open them on falstad.com, are written by `scripts/falstad_library.py` from the same "
            "parts; [falstad/README.md](falstad/README.md) lists the links and what differs from the SPICE circuits, and CI runs every link "
            "on falstad.com and compares it with the ideal equations (within 1 %).",
            "",
            "## 한국어",
            "",
            ...
            *rows_ko,
            ...
            "[falstad/README.md](falstad/README.md)에 링크와 SPICE 회로와의 차이가 있으며, CI는 모든 링크를 falstad.com에서 "
            "실행해 이상적인 식(1 % 이내)과 비교합니다.",
            "",
        ]
    )
...
def results_of(runs: dict[str, dict], version: str) -> dict:
    ...
    for c in CASES:
        ...
        out["cases"][c["id"]] = {
            "topology": c["topology"],
            "example": c["example"],
            "preset": {"id": c["preset"][0], "en": c["preset"][1], "ko": c["preset"][2]},
            "params": p,
            "cycles": c["cycles"],
            "quantities": {
    ...
    if args.check:
        if not RESULTS.exists():
            ...
            if all(c["id"] in old["cases"] for c in CASES):
                ...
                # sim/falstad/ is written and checked by scripts/falstad_library.py
                present = {p for p in SIM.rglob("*") if p.is_file() and (SIM / "falstad") not in p.parents}
