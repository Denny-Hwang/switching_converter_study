...
/**
 * The decimal a computed value stands for: twelve digits drop the binary noise
 * of the arithmetic (4.5² × 0.03 comes out as 0.60749999…), so that the value
 * rounds as that decimal does (0.6075 W to 608 mW, not 607 mW).
 */
const decimal = (x: number): number => Number(x.toPrecision(12));

function trim(x: number, sig: number): string {
  const s = decimal(x).toPrecision(sig);
...
export function formatSI(value: number, unit: string, sig = 3): string {
  ...
  const printed = Number(decimal(abs).toPrecision(sig));
  for (const [scale, prefix] of PREFIXES) {
    if (printed >= scale) {
      return `${minus(trim(value / scale, sig))} ${prefix}${unit}`;
    }
  }
  return `${minus(decimal(value).toExponential(sig - 1))} ${unit}`;
}
