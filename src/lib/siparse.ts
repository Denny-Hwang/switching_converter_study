 ...
 * another unit (100 µF in a field of henries) is not a number.
 */
...
const PREFIXES = 'meg|Meg|MEG|[fpnuµμmkKMGT]';
...
function withPrefix(t: string): number {
  const m = WITH_PREFIX.exec(t);
  if (!m) return Number.NaN;
  const exp = Number(m[2] ?? 0) + (m[3] ? PREFIX[m[3]]! : 0);
